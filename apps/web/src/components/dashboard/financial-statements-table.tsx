...
import { useFinancialStatements } from "@/hooks/use-financial-statements"
import { triggerFinancialStatementsCollection } from "@/lib/api"
...
export function FinancialStatementsTable({ className }: FinancialStatementsTableProps) {
  ...
  const handleRunCollection = async () => {
    ...
    try {
      const result = await triggerFinancialStatementsCollection()
      if (result.error) {
  ...
  return (
    <div className={cn("space-y-4", className)}>
      {/* Header with filter and period info */}
...
export function FinancialStatementsTableSkeleton({
  className,
}: {
  className?: string
}) {
