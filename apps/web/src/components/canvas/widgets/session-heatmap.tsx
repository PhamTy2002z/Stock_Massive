 ...
 * read alike.
 *
 * The same numbers are reachable as text: every cell carries a `<title>`, and
 * the table underneath is what a screen reader reads rather than the grid.
 */
 ...
 * four colours would invite a reader to look for four categories. `--chart-1`
 * is the app's lead accent and is defined for both themes, so the ladder holds
 * its contrast on either ground.
 */
const FILLS = [
  "hsl(var(--chart-1) / 0.18)",
  "hsl(var(--chart-1) / 0.42)",
  "hsl(var(--chart-1) / 0.68)",
  "hsl(var(--chart-1) / 0.95)",
] as const
...
export function SessionHeatmapWidget({ frame, options }: WidgetProps) {
  ...
  const width = ROW_LABEL + buckets.length * (CELL + GAP)
      ...
      <div className="overflow-x-auto scrollbar-thin">
        ...
        >
          {buckets.map((bucket, column) =>
            // Every other label, because seventeen four-character labels do not
            // fit fourteen pixels apiece and overlapping ticks read as noise.
            column % 2 === 0 ? (
              <text
                key={bucket.column}
                x={ROW_LABEL + column * (CELL + GAP)}
                y={HEADER - 6}
                className="fill-ink-5 text-[9px]"
              >
                {bucket.column}
              </text>
            ) : null,
