...
EXPECTED_CATALOG = {
    "web_search": ("web", registry.ToolEffect.READ, registry.ToolIdempotency.IDEMPOTENT, registry.ToolAccess.NETWORK, registry.ContentTrust.UNTRUSTED, registry.ToolConcurrency.PARALLEL_SAFE),
    "fetch_url": ("web", registry.ToolEffect.READ, registry.ToolIdempotency.IDEMPOTENT, registry.ToolAccess.NETWORK, registry.ContentTrust.UNTRUSTED, registry.ToolConcurrency.PARALLEL_SAFE),
    "session_search": ("memory", registry.ToolEffect.READ, registry.ToolIdempotency.IDEMPOTENT, registry.ToolAccess.STORE, registry.ContentTrust.TRUSTED_STRUCTURED, registry.ToolConcurrency.PARALLEL_SAFE),
    "remember_fact": ("memory", registry.ToolEffect.WRITE, registry.ToolIdempotency.UNKNOWN, registry.ToolAccess.STORE, registry.ContentTrust.TRUSTED_STRUCTURED, registry.ToolConcurrency.SERIALIZED),
    "recall_facts": ("memory", registry.ToolEffect.READ, registry.ToolIdempotency.IDEMPOTENT, registry.ToolAccess.STORE, registry.ContentTrust.TRUSTED_STRUCTURED, registry.ToolConcurrency.PARALLEL_SAFE),
}
...
def test_all_shipped_tools_declare_a_complete_behavior_contract():
    with isolated_registry():
        ...
        for entry in registry.entries():
            assert (entry.toolset, entry.effect, entry.idempotency, entry.access, entry.content_trust, entry.concurrency) == EXPECTED_CATALOG[entry.name]
...
async def test_one_read_only_registration_flows_through_generic_consumers():
    ...
    with isolated_registry():
        ...
        try:
            ...
            result = await ToolExecutor(context=registry.ToolContext(), surface=surface, availability=lambda _name: True).run([ToolCall(id="call-1", name=entry.name, arguments={"value": "x"})])
