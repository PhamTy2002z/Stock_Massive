...
def test_a_signal_desk_announcement_carries_the_contract_fields_and_never_a_number():
    """The whole arrangement in one assertion.

    Every open tab receives this event, so what it may carry is an allowlist:
    the id to fetch the picture by, enough to draw a skeleton of the right
    height, and enough for a reader with twenty boards to find this one again.
    A frame on this channel would put the numbers back where the Study contract
    exists to keep them out of.
    """
    published = publisher()

    event = published.signal_desk_ready(
        {
    ...
    assert set(event.data) == {
        "artifactId",
        "studyName",
        # The Vietnamese name, so no surface has to print the slug beside it.
        "studyDisplayName",
        "title",
        "blockCount",
        "round",
        # What a reader searches on when the strip cannot hold every board.
        "symbol",
        "asOf",
    }
    assert "frames" not in event.data
    assert "headline" not in event.data
