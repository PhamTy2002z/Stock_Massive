...
Và một điều đối chiếu ngược: **Hermes không có bộ chấm chất lượng đáp án nào.**
`batch_runner.py`/`mini_swe_runner.py` chỉ sinh trajectory, không có grader;
`verify/runner.py` chỉ chấm build/test/boot xanh hay không. Nên phần "biết mình
dở ở đâu" là thứ không thể port từ đây — nó phải được dựng, nếu muốn có.
