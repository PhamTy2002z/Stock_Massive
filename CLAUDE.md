...
- Agent (`src/agent/`) là **trợ lý tổng quát** trên khung kiểu Hermes (`registry` · `toolsets` · `definitions` · `executor` · `guardrails` · `budget` · `untrusted`). **Lane chat không đọc dữ liệu của hệ thống này**: nó chọn đúng hai bundle `web` + `memory` = 5 tool `web_search`, `fetch_url`, `session_search`, `remember_fact`, `recall_facts`. Ranh giới đó nằm ở `toolsets.CHAT_TOOLSETS` và được kiểm lúc import — `AgentLoop(toolsets=None)` mặc định về đúng tuple đó, **không** về "mọi bundle đã đăng ký". Thêm/đổi tool thì đi qua `registry`/`toolsets`/`definitions`, không hardcode trong `loop.py`.
- Bundle thứ ba, `signals` (`list_fields` + `get_field` ở `agent/tools/signals.py`, `check_price_claim` ở `agent/tools/price_check.py`), **chỉ lane Analysis chọn**. Hai tool store đọc Signal Field cho đúng `(symbol, trading_day)` mà `ToolContext` đưa vào — model không được đặt tên mã, phiên hay peer list. Đừng thêm nó vào `CHAT_TOOLSETS`.
