...
- Bundle thứ ba, `signals` (`list_fields` + `get_field`, `agent/tools/signals.py`), **chỉ lane Analysis chọn**. Nó đọc Signal Field từ store cho đúng `(symbol, trading_day)` mà `ToolContext` đưa vào — model không được đặt tên mã, phiên hay peer list. Đừng thêm nó vào `CHAT_TOOLSETS`.
