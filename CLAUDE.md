...
# Source of Truth — Investment Intelligence Harness
Khi đề xuất hoặc thay đổi AI, agent, model, tool, context, memory, eval, portfolio, risk hay proactive workflow, đọc `docs/Harness/README.md` trước và mở tài liệu SOT mà nó route tới. SOT sở hữu mục tiêu, target architecture và graduation gates; source/test sở hữu behavior hiện tại; `docs/hermes/` và `docs/opencode/` là evidence research, không phải kiến trúc để port nguyên.
...
- **"Chạy" và "trả về số" là hai việc.** `get_field` trả refusal như một *kết quả* model đọc được, nên `agent_tool_call.status` là `ok` cho cả ba loại: có số · `no_value:<signal issue>` · `cannot_read`. Cột `agent_tool_call.outcome` (và field `outcome` trên wire `tool.call`) là chỗ duy nhất phân biệt — đo được trước khi có nó: 151 lời gọi → 94 có số, 42 rỗng, 15 ngoài phạm vi, tất cả `ok`. Vốn từ ở `agent/messages.py::outcome_of`; rail hiện "Không có số"/"Ngoài phạm vi". Thêm tool có figure thì thêm tên vào `_FIGURE_TOOLS`, đừng đoán từ hình dạng payload.
- **Sàn của một percentile là hàm của mẫu, không phải hằng số**: `signals/fields.py::min_sample_for` = `max(ceil(0.6 × mẫu), 15)`, dùng chung cho cross-section và cho liquidity standing của gateway. Trước đó là hai hằng số 30 rời nhau, mỗi comment tự nhận "cùng một sàn" — với Universe đúng 30 mã thì một mã mới niêm yết giết mọi phân vị động lượng cho mọi mã. Sàn tính trên mẫu **được hỏi**, không trên mẫu sống sót.
- **`min_sessions` là sàn lịch sử, `lookback_sessions` là cửa sổ đọc** — `SignalField.window_sessions` giải hai cái. Bốn factor percentile có sàn 1 phiên và cửa sổ 21: FiinQuant chỉ ghi `market_cap_vnd` trên một phần rất nhỏ số phiên, và với cửa sổ bằng sàn thì vòng lùi trong `_market_cap` là dead code. Vốn hoá lấy từ phiên cũ hơn phiên cuối cửa sổ thì figure degraded `stale_market_cap` và mang cả `price_session` lẫn `window_session`.
- **Mã refusal phải trỏ đúng input thiếu.** `_quarterly_ratio` từng gộp ba nguyên nhân vào `fundamental_not_stored`, nên hai factor nói "chưa có báo cáo quý" trong khi báo cáo có sẵn và ROE trên đúng statement đó vẫn trả lời. Giờ: `fundamental_not_stored` (không có statement) · `statement_line_missing` (có statement, thiếu dòng) · `market_cap_absent` (thiếu vốn hoá, không phải thiếu filing). Thêm mã thì phải thêm câu ở **cả** `alpha/reasons.py` và `apps/web/src/lib/signal-issues.ts`.
...
- Đo vòng lặp Analysis: `GET /api/v1/ops/analysis-loop` (admin, chỉ đọc, không ngưỡng) trả ba số từ `alpha/analysis_reads.py` — substitution rate · round yield · tỷ lệ figure được dẫn. Không có bộ eval nào khác. Substitution rate cao **không** chứng minh Analysis đúng, và câu cảnh báo đó đi kèm trong response.
