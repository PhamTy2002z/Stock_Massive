...
5. **"Behavior contracts over snapshots"** cho eval/test — khớp thẳng với `docs/agents/eval-battery.md` đã có (Eval Gate), nhưng nên nói rõ trong `CLAUDE.md`/test convention: test invariant (quan hệ giữa field), không test giá trị cụ thể (danh sách model, version literal) — để test không vỡ mỗi lần thêm provider/model.
