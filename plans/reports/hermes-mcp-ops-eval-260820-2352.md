...
- **Từ `mcp_schema_cache.py`**: nếu `mcp_enabled` được bật trong tương lai (hiện tắt), pattern "cache manifest tool trên đĩa theo fingerprint(config) + TTL theo SEP-2549, để lazy-register mà không spawn subprocess lúc dashboard khởi động" giải quyết đúng vấn đề "Eval pin MCP off vì availability của server có thể move fixture" — cache lazy này KHÔNG che được vấn đề đó (fixture vẫn phải pin off vì output tool call vẫn phụ thuộc server thật lúc gọi), nhưng giúp UI/registry hiển thị tool list nhanh mà không tốn network mỗi lần mở app.
- **Eval Battery của Stock_Massive (ADR-0016, fixture đóng băng theo ngày+hash) làm tốt hơn cả `batch_runner.py`**: `batch_runner.py`/`mini_swe_runner.py` KHÔNG có oracle/grader — chỉ sinh trajectory, chấm điểm nằm ngoài hoàn toàn (phải nối vào 1 harness khác). Stock_Massive's `make eval` tự chấm điểm từng category + so diff baseline — kiến trúc chặt hơn cho mục đích "regression gate trước merge" mà đề bài mô tả (`docs/agents/eval-battery.md`, ADR-0016 "eval battery over a frozen fixture not CI"). Không cần port gì từ `batch_runner.py` cho mục đích này.
